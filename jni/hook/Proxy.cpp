...
static void proxyInvoker(const u4* args, JValue* pResult, const Method* method,
		Thread* self);
static bool mustWrapException(const Method* method, const Object* throwable);
static void initCommonClass();

static ClassObject* arrayObjectClass;
 ...
 */
ClassObject* dvmGenerateProxyClass(const char* className,
		const char* srcclass) {
	int result = -1;
	ArrayObject* throws = NULL;

	char* nameStr = (char*) className;
	if (nameStr == NULL) {
	 ...
	 */
	size_t newClassSize = sizeof(ClassObject)
			+ kProxySFieldCount * sizeof(StaticField);
	ClassObject* newClass = (ClassObject*) dvmMalloc(newClassSize,
			ALLOC_NON_MOVING);

	ClassObject* srcClass = dvmFindSystemClass(srcclass);
	CLEAR_CLASS_FLAG(srcClass, ACC_FINAL);
	SET_CLASS_FLAG(srcClass, ACC_PUBLIC);

	/* allocate storage */

	ArrayObject* interfaces = dvmAllocArrayByClass(arrayObjectClass, 1,
			ALLOC_NON_MOVING);
	if (interfaces == NULL)
		return NULL;
	Object** argObjects = (Object**) (void*) interfaces->contents;
	argObjects[0] = srcClass;
	Object* loader = srcClass->classLoader;

	if (newClass == NULL)
		goto bail;

	{
		ClassObject* classclazz = dvmFindSystemClass("Ljava/lang/Class;");
		DVM_OBJECT_INIT(newClass, classclazz);
		dvmSetClassSerialNumber(newClass);
		newClass->descriptorAlloc = dvmNameToDescriptor(nameStr);
		newClass->descriptor = newClass->descriptorAlloc;
		SET_CLASS_FLAG(newClass, ACC_PUBLIC | ACC_FINAL);
		dvmSetFieldObject((Object *) newClass,
				OFFSETOF_MEMBER(ClassObject, super), (Object *) srcClass);
		newClass->primitiveType = PRIM_NOT;
		dvmSetFieldObject((Object *) newClass,
				OFFSETOF_MEMBER(ClassObject, classLoader), (Object *) loader);
	}

	{
		ClassObject** interfaceclass =
				(ClassObject**) (void*) interfaces->contents;
		InstField* field = (InstField*) malloc(sizeof(InstField));
		newClass->ifieldCount = 1;
		newClass->ifieldRefCount = 1;
		field->name = "h";
		field->clazz = dvmFindSystemClass(
				"Ljava/lang/reflect/InvocationHandler;");
		field->signature = "Ljava/lang/reflect/InvocationHandler;";
		field->accessFlags = ACC_PUBLIC;
		dvmSetFieldObject((Object *) newClass,
				OFFSETOF_MEMBER(ClassObject, ifields), (Object *) field);
	}
	 ...
	 */
	{
		...
		if (!gatherMethods(interfaces, &methods, &throws, &methodCount)) {
			LOGE("Proxy gather methods failed");
			goto bail;
		}
		newClass->virtualMethodCount = methodCount;
		size_t virtualMethodsSize = methodCount * sizeof(Method);
		newClass->virtualMethods = (Method*) dvmLinearAlloc(
				newClass->classLoader, virtualMethodsSize);
		for (int i = 0; i < newClass->virtualMethodCount; i++) {
			createHandlerMethod(newClass, &newClass->virtualMethods[i],
					methods[i]);
		}
		free(methods);
...
void initCommonClass() {
	arrayObjectClass = dvmFindSystemClass("[Ljava/lang/Object;");
}
...
static bool gatherMethods(ArrayObject* interfaces, Method*** pMethods,
	...
	for (i = 0; i < numInterfaces; i++, classes++) {
		...
		for (j = 0; j < clazz->iftableCount; j++) {
			...
			LOGD("---  +%s %d", iclass->descriptor, iclass->virtualMethodCount);
	 ...
	 */
	{
		ClassObject* obj = dvmFindSystemClass("Ljava/lang/Object;");
		allMethods[0] = dvmFindVirtualMethodByDescriptor(obj, "equals",
				"(Ljava/lang/Object;)Z");
		allMethods[1] = dvmFindVirtualMethodByDescriptor(obj, "hashCode",
				"()I");
		allMethods[2] = dvmFindVirtualMethodByDescriptor(obj, "toString",
				"()Ljava/lang/String;");
		allCount = 3;
	}
	...
	for (i = 0; i < numInterfaces; i++, classes++) {
		...
		for (j = 0; j < clazz->virtualMethodCount; j++) {
			...
			CLEAR_METHOD_FLAG(&(clazz->virtualMethods[j]), ACC_FINAL);
			SET_METHOD_FLAG(&(clazz->virtualMethods[j]), ACC_PUBLIC);
		}
		...
		if (dupe) {
			if (best) {
				...
				if (commonThrows != NULL
					...
					ClassObject* cls = dvmFindSystemClass("[Ljava/lang/Class;");
					throwArray = dvmAllocArrayByClass(cls, commonCount,
							ALLOC_DEFAULT);
					if (throwArray == NULL) {
					...
					for (ent = 0; ent < commonCount; ent++) {
						contents[ent] = (Object*) dvmPointerSetGetEntry(
								commonThrows, ent);
					}
...
static bool createExceptionClassList(const Method* method,
	...
	bail: dvmReleaseTrackedAlloc((Object*) exceptionArray, NULL);
	return result;
}
...
static void createConstructor(ClassObject* clazz, Method* meth) {
	 ...
	 * this purpose.
	 */
	ClassObject* obj = dvmFindSystemClass("Ljava/lang/reflect/Proxy;");
	Method* srcMeth = dvmFindDirectMethodByDescriptor(obj,
			"constructorPrototype", "(Ljava/lang/reflect/InvocationHandler;)V");

	meth->clazz = clazz;
	meth->accessFlags = ACC_PUBLIC | ACC_NATIVE;
	meth->name = "<init>";
	meth->prototype = srcMeth->prototype;
	meth->shorty = srcMeth->shorty;
...
static ArrayObject* boxMethodArgs(const Method* method, const u4* args) {
	...
	/* allocate storage */
	ArrayObject* argArray = dvmAllocArrayByClass(arrayObjectClass, argCount,
			ALLOC_DEFAULT);
	if (argArray == NULL)
		return NULL;
...
static void proxyConstructor(const u4* args, JValue* pResult,
	...
	InstField* f = dvmFindInstanceField(method->clazz, "h",
			"Ljava/lang/reflect/InvocationHandler;");
	dvmSetFieldObject(obj, f->byteOffset, handler);
}
...
static void proxyInvoker(const u4* args, JValue* pResult, const Method* method,
	...
	InstField* f = dvmFindInstanceField(method->clazz, "h",
			"Ljava/lang/reflect/InvocationHandler;");
	handler = dvmGetFieldObject(thisObj, f->byteOffset);
	 ...
	 */
	invoke =
			dvmFindVirtualMethodHierByDescriptor(handler->clazz, "invoke",
	...
	LOGD("invoke: %s.%s, this=%p, handler=%s",
			method->clazz->descriptor, method->name, thisObj, handler->clazz->descriptor);
	 ...
	 */
	dvmCallMethod(self, invoke, handler, &invokeResult, thisObj, methodObj,
			argArray);
	if (dvmCheckException(self)) {
	...
	bail: dvmReleaseTrackedAlloc(methodObj, self);
