...
public class Proxy implements Serializable {
     ...
     *             {@code null}
     */
    private static Class<?> getProxyClass(ClassLoader loader, Class<?>... interfaces) throws IllegalArgumentException {
        ...
        synchronized (loaderCache) {
            ...
            if (ref == null) {
                ...
                if (commonPackageName != null && commonPackageName.length() > 0) {
                    ...
                    nextClassName = interfaces[0].getName() + "." + nextClassName;
                }
                if (loader == null) {
                    loader = ClassLoader.getSystemClassLoader();
                }
                try {
                    newClass = Class.forName(nextClassName, true, interfaces[0].getClassLoader());
                } catch (ClassNotFoundException e) {
                    generateProxyClass(nextClassName.replace('.', '/'), "L" + interfaces[0].getName().replace('.', '/') + ";");
                    try {
                        newClass = Class.forName(nextClassName, true, interfaces[0].getClassLoader());
                    } catch (ClassNotFoundException e1) {
                        Log.e(Hook.TAG, "get proxy failed", e1);
                    }
                }
     ...
     */
    public static Object newProxyInstance(ClassLoader loader, Class<?> classes, InvocationHandler h) throws IllegalArgumentException {
        if (h == null) {
            throw new NullPointerException("h == null");
        }
        Class<?>[] interfaces = new Class<?>[] { classes };
        try {
            return getProxyClass(loader, interfaces).getConstructor(new Class<?>[] { InvocationHandler.class }).newInstance(
                    new Object[] { h });
    ...
    public native static void generateProxyClass(String classDescription, String srcClass);
}
